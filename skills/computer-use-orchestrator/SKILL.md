...
> **硬规则**：若 `targets` / `list_windows` / `win_list_apps` 把某窗口标注为 **🌐浏览器**，
> 且任务是该网页内的操作（点链接/按钮、填表单、登录登出）→ **必须优先用浏览器 DOM 工具**
> （有 aria-label/role，比纯视觉点击稳得多）；computer use 仅在浏览器工具不可用时兜底。
...
| **Windows** 主机此刻开着的程序，且**不想被抢鼠标** | **`win_*` UIA 元素级**（不抢鼠标，假鼠标+高亮可观测） | ✅（同一桌面、不占用鼠标）|
| 用户主机此刻开着的程序（其它情况） | **主桌面直跑**（视觉坐标） | ❌（共用用户鼠标）|
